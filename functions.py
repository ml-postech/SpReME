...
from generate_data import gen_data, predict_data, cal_abs_max
...
def update_coef(mask_threshold, degree, coef, mask, train_X, train_T, optimizer, abs_max):

    error = predict_for_train(degree, train_X, train_T, ((torch.sigmoid(mask) > mask_threshold).float() * coef), abs_max) 
...
def update_mask(degree, mask_schedule, reg, env_num, coef, mask, train_X, train_T, mask_optimizer, epo, abs_max):
    mask_wo_reg = predict_for_valid(degree, train_X, train_T, coef * torch.sigmoid(mask), abs_max).sum()
    mask_cost = mask_wo_reg + env_num*reg*((1.0+mask_schedule)**((epo-1)))* torch.norm(torch.sigmoid(mask), p=1)
...
def print_mask_info(mask_threshold, mask):
    sparsity = (((mask > mask_threshold).float() == 0.0).float().sum()) / (mask.shape[0] * mask.shape[1])
...
def cal_err(mask_threshold, degree, coef, mask, X_data, eval_time, abs_max):        
    return predict_for_valid(degree, X_data, eval_time, ((torch.sigmoid(mask) > mask_threshold).float() * coef), abs_max).mean()

def cal_mask_perf(data_type, mask_threshold, mask):
    mask_gt = get_mask_gt(data_type, mask)
    precision = (
        torch.logical_and(mask_gt, (mask > mask_threshold).float()).float().sum() / (mask > mask_threshold).float().sum())  
    recall = (
        torch.logical_and(mask_gt, (mask > mask_threshold).float()).float().sum() / mask_gt.sum()) 
...
def args_set(args):
    if args.data_type == "3d":
        ...
        if args.set_args == 2:
            args.coef_lr = 0.1
            args.coef_threshold = 0.001
            args.mask_lr = 0.1
            args.mask_threshold = 1e-5
            args.reg = 10
...
def predict_for_train(degree, X_data, eval_time, masked_coef, abs_max):
    ...
    for env_idx in range(masked_coef.shape[0]):
        for i in range(len(eval_time)-1):
            ...
            loss[env_idx] += F.smooth_l1_loss(x_train.permute(1,0,2), X_data[:,env_idx, i:i+2 ,:], reduction = 'sum')
...
def predict_for_valid(degree, gt, eval_time, masked_coef, abs_max):
    ...
    for env_idx in range(masked_coef.shape[0]):
        ...
        loss[env_idx] += F.mse_loss(x_train.permute(1,0,2), gt[:,env_idx,:,:], reduction = 'sum')
    
    traj, env, time, state = gt.shape
    
    return loss/(traj*time*state)

def init_by_SINDY(data_type, degree, dt, X, train_T, val_X, val_T, abs_max, train_X_dots):
    ...
    for env, train_X in enumerate(X):
        ...
        for threshold in [0.1, 0.05, 0.01]:
            for alpha in [1, 0.5, 0.1, 0.05]:
                ...
                partial(
                    predicted_func_clip,
                    param=torch.tensor(model.coefficients().T*np.expand_dims(abs_max[env],axis= -1)).to('cuda').float(),
                    deg=degree,
                    abs_max = abs_max[env]
                ),
                val_X[:,env, 0,:],
...
def init_mask(coef):
    ...
    union = torch.sum((coef != 0).float(), dim=0) != 0
    inter = torch.sum((coef != 0).float(), dim=0) == coef.shape[0]
